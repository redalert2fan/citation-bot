...
final class TemplatePart1Test extends testBaseClass {
    ...
    public function testNoGoonUTF8(): void {
        $text = "{{cite news |date=ÃƒÂ£Ã‚ÂÃ‚Â³ÃƒÂ£Ã‚ÂÃ‚Â£ÃƒÂ£Ã‚ÂÃ‚ÂÃƒÂ£Ã¢â‚¬Å¡Ã…Â ÃƒÂ¯Ã‚Â¼Ã¢â‚¬ËœÃƒÂ¤Ã‚Â½Ã‚Â ÃƒÂ§Ã¢â€žÂ¢Ã‚Â½ÃƒÂ©Ã‚Â´Ã…Â½|title=ÃƒÂ©Ã‹Å“Ã‚ÂªÃƒÂ§Ã‚Â¥Ã…Â¾ÃƒÂ£Ã‚ÂÃ‚Â³ÃƒÂ£Ã‚ÂÃ‚Â£ÃƒÂ£Ã‚ÂÃ‚ÂÃƒÂ£Ã¢â‚¬Å¡Ã…Â ÃƒÂ¯Ã‚Â¼Ã¢â‚¬ËœÃƒÂ¤Ã‚Â½Ã‚Â ÃƒÂ§Ã¢â€žÂ¢Ã‚Â½ÃƒÂ©Ã‚Â´Ã…Â½ÃƒÂ¥Ã‚Â¤Ã‚Â§ÃƒÂ£Ã†â€™Ã‚Â»ÃƒÂ¥Ã‚Â¤Ã‚Â§ÃƒÂ¥Ã‚Â±Ã‚Â±ÃƒÂ£Ã¢â€šÂ¬Ã‚ÂÃƒÂ©Ã¢â‚¬Â°Ã¢â‚¬Å¾ÃƒÂ¤Ã‚ÂºÃ‚ÂºÃƒÂ©Ã‚Â­Ã¢â‚¬Å¡ÃƒÂ£Ã‚ÂÃ‚Â®ÃƒÂ¦Ã…â€™Ã‚ÂÃƒÂ£Ã‚ÂÃ‚Â¡ÃƒÂ¤Ã‚Â¸Ã‚Â»ÃƒÂ£Ã‚ÂÃ‚Â |journal=ÃƒÂ©Ã¢â‚¬Â°Ã¢â‚¬Å¾ÃƒÂ¤Ã‚ÂºÃ‚ÂºÃƒÂ©Ã‚Â­Ã¢â‚¬Å¡}}";
        $expanded = $this->process_citation($text);
        $this->assertSame($text, $expanded->parsed_text());
    }
    ...
    public function testGetDoiFromCrossref(): void {
        $text = '{{Cite journal | last1 = Glaesemann | first1 = K. R. | last2 = Fried | first2 = L. E. | doi = | title = Improved woodÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“kirkwood detonation chemical kinetics | journal = Theoretical Chemistry Accounts | volume = 120 | pages = 37ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“43 | year = 2007 |issue=1ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“3|pmid=<!-- -->|pmc=<!-- -->|arxiv=<!-- -->|bibcode=<!-- -->}}';
        $expanded = $this->process_citation($text);
    ...
    public function testChangeNothing3(): void {
        $text = '{{cite journal |title=The tumbling rotational state of 1I/ÃƒÂ¢Ã¢â€šÂ¬Ã‹Å“Oumuamua<!-- do not change odd punctuation--> |journal=Nature title without caps <!-- Deny Citation Bot-->    |pages=383-386 <!-- do not change the dash--> }}';
        $expanded = $this->process_page($text);
        $this->assertSame($text, $expanded->parsed_text());
    }

    public function testNoLoseUrl(): void {
        $text = '{{cite book |last=SÃƒÆ’Ã‚Â¶derstrÃƒÆ’Ã‚Â¶m |first=Ulrika |date=2015 |title=Sandby Borg: Unveiling the Sandby Borg Massacre |url= |location= |publisher=Kalmar lÃƒâ€žÃ‚Âns museum |isbn=9789198236620 |language=Swedish }}';
        $expanded = $this->process_page($text);
        $this->assertSame($text, $expanded->parsed_text());
    }
    ...
    public function testBadAuthor2(): void {
        $text = '{{cite journal|title=Guidelines for the management of adults with hospital-acquired, ventilator-associated, and healthcare-associated pneumonia |journal=Am. J. Respir. Crit. Care Med. |volume=171 |issue=4 |pages=388ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“416 |year=2005 |pmid=15699079 |doi=10.1164/rccm.200405-644ST|pmid=|pmc=|arxiv=}}';
        $expanded = $this->process_citation($text);
    ...
    public function testTemplateRenamingURLConvert(): void {
        $text = '{{Cite journal|url=http://www.sciencedirect.com/science/article/pii/B9780123864543000129|last=Roberts|first=L.|date=2014|publisher=Academic Press|isbn=978-0-12-386455-0|editor-last=Wexler|editor-first=Philip|location=Oxford|pages=993ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“995|doi=10.1016/b978-0-12-386454-3.00012-9|pmid=<!-- -->|pmc=<!-- -->|arxiv=<!-- -->}}';
        $expanded = $this->process_citation($text);
    ...
    public function testRemoveWikilinks4c(): void {
        $expanded = $this->process_citation("{{cite journal|journal=[[Bulletin du MusÃƒÆ’Ã‚Â©um national dÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢Histoire naturelle, Paris]]}}");
        $this->assertSame("[[Bulletin du MusÃƒÆ’Ã‚Â©um national d'Histoire naturelle, Paris]]", $expanded->get2('journal'));
    }

    public function testRemoveWikilinks4d(): void {
        $expanded = $this->process_citation("{{cite journal|journal=[[Bulletin du MusÃƒÆ’Ã‚Â©um national dÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢Histoire naturelle, Paris|Hose]]}}");
        $this->assertSame("[[Bulletin du MusÃƒÆ’Ã‚Â©um national d'Histoire naturelle, Paris|Hose]]", $expanded->get2('journal'));
    }
    ...
    public function testRemoveWikilinks7c(): void {
        $text = "{{Cite journal|last=[[Nelarine Cornelius|Cornelius]]|first= [[Nelarine Cornelius|Nelarine]]|last2= Todres|first2= Mathew|last3= Janjuha-Jivraj|first3= Shaheena|last4= Woods|first4= Adrian|last5= Wallace|first5= James|date= 2008|title= Corporate Social Responsibility and the Social Enterprise|jstor= 25482219|journal= Journal of Business Ethics|volume= 81|issue= 2|pages= 355ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“370|doi= 10.1007/s10551-007-9500-7|s2cid= 154580752|url = <!-- dsfasdfds -->|pmid=<!-- -->|pmc=<!-- -->|arxiv=<!-- -->}}";
        $expanded = $this->process_citation($text);
        $this->assertNull($expanded->get2('last'));
        $this->assertNull($expanded->get2('first'));
    ...
    public function testScriptTitle(): void {
        $text = "{{cite book |author={{noitalic|{{lang|zh-hans|ÃƒÂ¥Ã¢â‚¬ÂºÃ‚Â½ÃƒÂ¥Ã…Â Ã‚Â¡ÃƒÂ©Ã¢â€žÂ¢Ã‚Â¢ÃƒÂ¤Ã‚ÂºÃ‚ÂºÃƒÂ¥Ã‚ÂÃ‚Â£ÃƒÂ¦Ã¢â€žÂ¢Ã‚Â®ÃƒÂ¦Ã…Â¸Ã‚Â¥ÃƒÂ¥Ã…Â Ã…Â¾ÃƒÂ¥Ã¢â‚¬Â¦Ã‚Â¬ÃƒÂ¥Ã‚Â®Ã‚Â¤ÃƒÂ£Ã¢â€šÂ¬Ã‚ÂÃƒÂ¥Ã¢â‚¬ÂºÃ‚Â½ÃƒÂ¥Ã‚Â®Ã‚Â¶ÃƒÂ§Ã‚Â»Ã…Â¸ÃƒÂ¨Ã‚Â®Ã‚Â¡ÃƒÂ¥Ã‚Â±Ã¢â€šÂ¬ÃƒÂ¤Ã‚ÂºÃ‚ÂºÃƒÂ¥Ã‚ÂÃ‚Â£ÃƒÂ¥Ã¢â‚¬â„¢Ã…â€™ÃƒÂ§Ã‚Â¤Ã‚Â¾ÃƒÂ¤Ã‚Â¼Ã…Â¡ÃƒÂ§Ã‚Â§Ã¢â‚¬ËœÃƒÂ¦Ã…Â Ã¢â€šÂ¬ÃƒÂ§Ã‚Â»Ã…Â¸ÃƒÂ¨Ã‚Â®Ã‚Â¡ÃƒÂ¥Ã‚ÂÃ‚Â¸ÃƒÂ§Ã‚Â¼Ã¢â‚¬â€œ}}}} |date=2012 |script-title=zh:ÃƒÂ¤Ã‚Â¸Ã‚Â­ÃƒÂ¥Ã¢â‚¬ÂºÃ‚Â½2010ÃƒÂ¥Ã‚Â¹Ã‚Â´ÃƒÂ¤Ã‚ÂºÃ‚ÂºÃƒÂ¥Ã‚ÂÃ‚Â£ÃƒÂ¦Ã¢â€žÂ¢Ã‚Â®ÃƒÂ¦Ã…Â¸Ã‚Â¥ÃƒÂ¥Ã‹â€ Ã¢â‚¬Â ÃƒÂ¥Ã…Â½Ã‚Â¿ÃƒÂ¨Ã‚ÂµÃ¢â‚¬Å¾ÃƒÂ¦Ã¢â‚¬â€œÃ¢â€žÂ¢ |location=Beijing |publisher={{noitalic|{{lang|zh-hans|ÃƒÂ¤Ã‚Â¸Ã‚Â­ÃƒÂ¥Ã¢â‚¬ÂºÃ‚Â½ÃƒÂ§Ã‚Â»Ã…Â¸ÃƒÂ¨Ã‚Â®Ã‚Â¡ÃƒÂ¥Ã¢â‚¬Â¡Ã‚ÂºÃƒÂ§Ã¢â‚¬Â°Ã‹â€ ÃƒÂ§Ã‚Â¤Ã‚Â¾}}}} [China Statistics Press] |page= |isbn=978-7-5037-6659-6 }}";
        $expanded = $this->process_citation($text);
    ...
    public function testParameterAlias(): void {
        ...
        |journal=Lichenologist |volume=23 |pages=309ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“321 |doi=10.1017/S0024282991000452 |issue=3|pmid=<!-- -->|pmc=<!-- -->|arxiv=<!-- -->}}';
        $expanded = $this->process_citation($text);
    ...
    public function testMisspeltParameters1(): void {
        $text = "{{Cite journal | ahtour=S.-X. HU, M.-Y. ZHU, F.-C. ZHAO, and M. STEINER|tutle=A crown group priapulid from the early Cambrian Guanshan LagerstÃƒÆ’Ã‚Â¤tte,|jrounal=Geol. Mag.|year= 2017.}}";
        $expanded = $this->process_citation($text);
    ...
    public function testMisspeltParameters2(): void {
        $text = "{{Cite journal | ahtour=S.-X. HU, M.-Y. ZHU, F.-C. ZHAO, and M. STEINER|tutel=A crown group priapulid from the early Cambrian Guanshan LagerstÃƒÆ’Ã‚Â¤tte,|jrounal=Geol. Mag.|year= 2017.}}";
        $expanded = $this->process_citation($text);
        $this->assertNotNull($expanded->get2('author')); ## Check: the parameter might be broken down into last1, first1 etc
    ...
    public function testId2Param5(): void {
        $text = '{{cite book|pages=1ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“2|id={{arxiv|astr.ph|1234.5678}}}}{{cite book|pages=1ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“3|id={{arxiv|astr.ph|1234.5678}}}}'; // Two of the same sub-template, but in different templates
        $expanded = $this->process_page($text);
        $this->assertSame('{{cite book|pages=1ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“2|arxiv=astr.ph/1234.5678 }}{{cite book|pages=1ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“3|arxiv=astr.ph/1234.5678 }}', $expanded->parsed_text());
    }
    ...
    public function testNestedTemplates1(): void {
        ...
        $expanded = $this->process_citation($text);
        $text = str_replace("-", "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“", $text); // Should not change anything other than upgrade dashes
        $this->assertSame($text, $expanded->parsed_text());
    }
