...
final class ZoteroTest extends TestBaseClass {
    ...
    public function testDropSomeURLEquivs9(): void {
        $text = "{{cite journal|url=https://pubs.acs.org/doi/10.1021/acs.analchem.8b04567|doi=10.1021/acs.analchem.8b04567|doi-access=free|pmid=30741529|pmc=6526953|title=ISiCLE: A Quantum Chemistry Pipeline for Establishing in Silico Collision Cross Section Libraries|journal=Analytical Chemistry|volume=91|issue=7|pages=4346Ã¢â‚¬â€œ4356|year=2019|last1=Colby|first1=Sean M.|last2=Thomas|first2=Dennis G.|last3=NuÃƒÂ±ez|first3=Jamie R.|last4=Baxter|first4=Douglas J.|last5=Glaesemann|first5=Kurt R.|last6=Brown|first6=Joseph M.|last7=Pirrung|first7=Meg A.|last8=Govind|first8=Niranjan|last9=Teeguarden|first9=Justin G.|last10=Metz|first10=Thomas O.|last11=Renslow|first11=Ryan S.}}";
        $template = $this->make_citation($text);
        $tmp_array = [$template];
        drop_urls_that_match_dois($tmp_array);
        $this->assertNull($template->get2('url'));
    ...
    public function testZoteroResponse27(): void {
        ...
        $url = '';
        $zotero_data = [];
        $zotero_data[0] = (object) ['title' => "Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½JunkÃ¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½Ã¯Â¿Â½", 'itemType' => 'webpage'];
        $zotero_response = json_encode($zotero_data);
        Zotero::process_zotero_response($zotero_response, $template, $url, $access_date);
    ...
    public function testZoteroResponse39(): void {
        $text = '{{cite journal|url=https://www.sciencedirect.com/science/article/pii/S0024379512004405|title=Geometry of the Welch bounds|journal=Linear Algebra and Its Applications|volume=437|issue=10|pages=2455Ã¢â‚¬â€œ2470|year=2012|last1=Datta|first1=S.|last2=Howard|first2=S.|last3=Cochran|first3=D.}}';
        $template = $this->make_citation($text);
        $access_date = 0;
    ...
    public function testZoteroResponse44(): void {
        ...
        $this->assertSame('34Ã¢â‚¬â€œ55', $template->get2('pages'));
    }
    ...
    public function testEnDashTaglineStripped(): void {
        ...
        $zotero_data = [];
        $zotero_data[0] = (object) ['title' => 'Vietjet to launch new route', 'itemType' => 'webpage', 'publicationTitle' => 'VnExpress International Ã¢â‚¬â€œ Latest news, business, travel and analysis from Vietnam'];
        $zotero_response = json_encode($zotero_data);
        Zotero::process_zotero_response($zotero_response, $template, $url, $access_date);
    ...
    public function testPipeTaglineStripped(): void {
        ...
        $zotero_data = [];
        $zotero_data[0] = (object) ['title' => 'Enhypen pre-orders', 'itemType' => 'webpage', 'publicationTitle' => 'Ã«â€â€Ã¬Å Â¤Ã­Å’Â¨Ã¬Â¹Ëœ | Ã«â€°Â´Ã¬Å Â¤Ã«Å â€ Ã­Å’Â©Ã­Å Â¸Ã«â€¹Â¤!'];
        $zotero_response = json_encode($zotero_data);
        Zotero::process_zotero_response($zotero_response, $template, $url, $access_date);
        $this->assertSame('Ã«â€â€Ã¬Å Â¤Ã­Å’Â¨Ã¬Â¹Ëœ', $template->get2('work'));
    }
    ...
    public function testSportsworldiPublicationTitle(): void {
        ...
        $zotero_data = [];
        $zotero_data[0] = (object) ['title' => 'ÃªÂ°â‚¬Ã¬Ââ€žÃ«Â¡Å“ ÃªÂ°â‚¬Ã«Å â€ ÃªÂ¸Â°Ã¬Â°Â¨, 25Ã¬ÂÂ¼ Ã¬Â»Â´Ã«Â°Â±Ã¢â‚¬Â¦Ã«â€˜Â Ã«Â²Ë†Ã¬Â§Â¸ Ã¬â€¹Â±ÃªÂ¸â‚¬ \'Ã«â€¹Â¤Ã¬â€¹Å“ Ã¬ÂÂ´Ã«Â³â€ž\' Ã«Â°Å“Ã­â€˜Å“', 'itemType' => 'newspaperArticle', 'publicationTitle' => 'ÃªÂ°â‚¬Ã¬Ââ€žÃ«Â¡Å“ ÃªÂ°â‚¬Ã«Å â€ ÃªÂ¸Â°Ã¬Â°Â¨, 25Ã¬ÂÂ¼ Ã¬Â»Â´Ã«Â°Â±Ã¢â‚¬Â¦Ã«â€˜Â Ã«Â²Ë†Ã¬Â§Â¸ Ã¬â€¹Â±ÃªÂ¸â‚¬ "Ã«â€¹Â¤Ã¬â€¹Å“ Ã¬ÂÂ´Ã«Â³â€ž" Ã«Â°Å“Ã­â€˜Å“ Ã¬Å Â¤Ã­ÂÂ¬Ã¬Â¸Â Ã¬â€ºâ€Ã«â€œÅ“'];
        $zotero_response = json_encode($zotero_data);
        Zotero::process_zotero_response($zotero_response, $template, $url, $access_date);
        $this->assertSame('Ã¬Å Â¤Ã­ÂÂ¬Ã¬Â¸Â Ã¬â€ºâ€Ã«â€œÅ“', $template->get2('work'));
    }
    ...
    public function testZoteroKoreanLanguage(): void {
        $this->requires_zotero(function (): void {
            ...
            $expanded = $this->expand_via_zotero($text);
            if ($expanded->get2('title') === 'Ã­ÂÂÃ«Â¸Å’ Ã¬Â¸Â¡ "Ã­ÂÂ¬Ã«Â¯Â¸Ã«â€¹â€º Ã¬â€šÂ¬Ã¬â€¹Â¤Ã¬Æ’Â Ã­â€¢Â´Ã¬Â²Â´, Ã­Å’â‚¬ Ã¬Â¡Â´Ã¬â€ Â Ã¬â€“Â´Ã«Â Â¤Ã¬â€ºÅ’"') {
